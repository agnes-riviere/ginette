...
temp_file <- grep(paste0("^", chaine_temp_file), fichiers, value = TRUE)
...
t_dates <- seq(
  from = as.POSIXct(paste0(as.Date(t_time[1]), " 00:00"), format = "%Y-%m-%d %H:%M"),
  to = as.POSIXct(paste0(as.Date(t_time[length(t_time)]), " 00:00"), format = "%Y-%m-%d %H:%M"),
  by = "1 day"
)
