...
flux_ts<-function(  namePointT='T3_Point3Nelly_14_04_22.csv',namePointP='P2_Point3Nelly_14_04_22.csv',sim_name=1,date_begin='14/04/2022 17:45:00')
  ...
  pathFile_heat_flx = paste0(path_output,'Sim_heat_flux_profil_t_',sim_name,'.dat')
  pathFile_darcy_flx = paste0(path_output,'Sim_velocity_profil_t_',sim_name,'.dat')  
  data_flux_raw_temp <- fread(file = pathFile_heat_flx,sep=' ',header = F)
  data_flux_raw_darcy <- fread(file = pathFile_darcy_flx,sep=' ',header = F)
  # colonne 1 : date
  ...
  data_flux_raw <- data.frame(data_flux_raw_temp[,1:4])
  data_flux_raw<-data.frame(data_flux_raw,data_flux_raw_darcy$V3)
  ...
  idx_pos2neg <- 1 + which(diff(data_meas$isPosTherm) == F)
  idx_neg2pos <- 1 + which(diff(data_meas$isPosTherm) == T)
  ...
  axis(side = 4,col='black',col.axis = 'salmon3')
  abline(h=0,lty=2,col='grey')
  mtext(text = expression('vertical heat flux [W/m'^'2'*']'),
        side = 4,line = 2.5,col='salmon3',cex=1.5)
  
  legend('topright',lty=c(1,2),col = c('salmon3','blue'),text.col= c('salmon3','blue'),
  ...
  abline(v=dates_perDay,lty=2,col='lightblue')
  # replot Darcy velocity on top
  lines(x = data_flux_raw$dates,y=data_flux_raw$cond,col='blue',lty=3)
  lines(x = data_flux_raw$dates,y=data_flux_raw$adv,col='salmon3')
  # replot box around plot
  box()
  
  legend('topright',lty=c(1,1),col = c('salmon3','blue'),text.col=c('salmon3','blue'),
