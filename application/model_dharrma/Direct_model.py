...
def setup_maillage_ginette(al,ala,az,imaille,dx,dy,dz,nclog,nllog,nrlog,dimen):
    
    #-----------------------------------------------------------------
    ## write the setup of the modeled domain
    f_param_bck = open("E_parametreV2_backup.dat", "r")
    f_param_new = open("E_parametreV2.dat", 'w')
    setup_model = f_param_bck.read()
    setup_model = setup_model.replace('[al]', '%s' % al)
    setup_model = setup_model.replace('[ala]', '%s' % ala)
    setup_model = setup_model.replace('[az]', '%s' % az)
    setup_model = setup_model.replace('[imaille]', '%1i' % imaille)
    setup_model = setup_model.replace('[dx]', '%s' % dx)
    setup_model = setup_model.replace('[dy]', '%s' % dy)
    setup_model = setup_model.replace('[dz]', '%s' % dz)
    setup_model = setup_model.replace('[nclog]', '%5i' % nclog)
    setup_model = setup_model.replace('[nllog]', '%5i' % nllog)
    setup_model = setup_model.replace('[nrlog]', '%5i' % nrlog)
    setup_model = setup_model.replace('[dimen]', '%s' % dimen)
...
def setup_CL_ginette(i_g,v_g,i_d,v_d,i_h,v_h,i_b,v_b,i_av,v_av,i_ar,v_ar):
    
    #-----------------------------------------------------------------
    ## write the setup of the modeled domain
    f_param_bck = open("E_cdt_aux_limites_backup.dat", "r")
    f_param_new = open("E_cdt_aux_limites.dat", 'w')
    setup_model = f_param_bck.read()
    setup_model = setup_model.replace('[icl_gauche]', '%1i' % i_g)
    setup_model = setup_model.replace('[valcl_gauche]', '%s' % v_g)
    setup_model = setup_model.replace('[icl_droite]', '%1i' % i_d)
    setup_model = setup_model.replace('[valcl_droite]', '%s' % v_d)
    setup_model = setup_model.replace('[icl_haut]', '%1i' % i_h)
    setup_model = setup_model.replace('[valcl_haut]', '%s' % v_h)
    setup_model = setup_model.replace('[icl_bas]', '%1i' % i_b)
    setup_model = setup_model.replace('[valcl_bas]', '%s' % v_b)
    setup_model = setup_model.replace('[icl_avant]', '%1i' % i_av)
    setup_model = setup_model.replace('[valcl_avant]', '%s' % v_av)
    setup_model = setup_model.replace('[icl_arriere]', '%1i' % i_ar)
    setup_model = setup_model.replace('[valcl_arriere]', '%s' % v_ar)
...
import matplotlib
def attribution_couleur(data):

    '''
    Fonction qui prend en entrée une liste de données et va lui attribuer une couleur pour chaque valeur
    unique contenue dans la liste. La fonction renvoie enfin une liste contenant des str représentant 
    les couleurs.

        Parameters:
    - data: Liste contenant les données que l'on veut représenter

    Returns:
    - data_color: Liste contenant la couleur associé de la valeur contenue dans data

    '''
    
    data_color = list()
    valeur_data = list(set(data)) # Permet d'obtenir toute les valeurs unique contenue dans data
    colors = list(matplotlib.colors.CSS4_COLORS.keys())
    
    for i in data:
        for j in range(0,len(valeur_data)):
            if i == valeur_data[j]:
                data_color.append(str(colors[j*10]))
    return data_color

def setup_ginette_DHARRMA(dt, ith, nb_day, z_top, z_bottom, az, dz, itsortie,wt_depth):
    
    entier_z_top, puissance_z_top = fct.decomposition_nombre(z_top)
    entier_z_bottom, puissance_z_bottom = fct.decomposition_nombre(z_bottom)
    entier_az, puissance_az = fct.decomposition_nombre(az)
    entier_dz, puissance_dz = fct.decomposition_nombre(dz)
    entier_wt, puissance_wt = fct.decomposition_nombre(wt_depth)
    # number of cell
    nb_cell=az/dz
    #-----------------------------------------------------------------
    ## write the setup of the modeled domain
    f_param_bck = open(f"{dossier_actuel}/input_ginette/E_parametre_backup.dat", "r")
    f_param_new = open(f"{dossier_actuel}/input_ginette/E_parametre.dat", 'w')
    setup_model = f_param_bck.read()
    setup_model = setup_model.replace('[dt]', '%05.0fD+00' % dt)
    setup_model = setup_model.replace('[ith]', '%1i' % ith)
    setup_model = setup_model.replace('[nitt]', '%010.0f' % nb_day)
    setup_model = setup_model.replace('[reptop]', '%05.0f' % entier_z_top + 'D-' + '%02i' %abs(puissance_z_top))
    setup_model = setup_model.replace('[repbot]', '%05.0f' % entier_z_bottom + 'D-' + '%02i' %abs(puissance_z_bottom))
    setup_model = setup_model.replace('[az]', '%05.0f' % entier_az + 'D-' + '%02i' %abs(puissance_az))
    setup_model = setup_model.replace('[dz]', '%04.0f' % entier_dz + 'D-' + '%02i' %abs(puissance_dz))
    setup_model = setup_model.replace('[itsortie]', '%08.0f' % itsortie)
    f_param_new.write(setup_model)
    f_param_bck.close()
    f_param_new.close()

    f_cdt_init_bck = open(f"{dossier_actuel}/input_ginette/E_cdt_initiale_backup.dat", "r")
    f_cdt_init_new = open(f"{dossier_actuel}/input_ginette/E_cdt_initiale.dat", 'w')
    setup_cdt = f_cdt_init_bck.read()
    if wt_depth < 0 :
        setup_cdt = setup_cdt.replace('[signe]', '-')
    else :
        setup_cdt = setup_cdt.replace('[signe]', '0')

    if puissance_wt < 0 :
        setup_cdt = setup_cdt.replace('[signe_exp]', '-')
    else :
        setup_cdt = setup_cdt.replace('[signe_exp]', '+')
    setup_cdt = setup_cdt.replace('[charge]', '%04i' % abs(entier_wt))
    setup_cdt = setup_cdt.replace('[exp]', '%02i' % abs(puissance_wt))
    f_cdt_init_new.write(setup_cdt)
    f_cdt_init_bck.close()
    f_cdt_init_new.close()


    print("Setup Ginette Ok")

def generate_zone_parameters_DHARRMA(thermique,depth_bottom,depth_top, dz, k1, phi1, alphaVG1, nVG1, Swr1, lambda1, C1, rho1):
    """
    Écrit le fichier de paramètres de zone en fonction du nombre de zones et des valeurs de paramètres.
    ...
    # Lire le fichier de sauvegarde des paramètres de zone
    with open(f"{dossier_actuel}/input_ginette/E_zone_parameter_backup.dat", "r") as f_paramZ_bck:
        param_zone = f_paramZ_bck.read()
    if thermique:
        # Remplacer les valeurs des paramètres
