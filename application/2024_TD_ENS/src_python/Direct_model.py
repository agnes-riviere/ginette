...
def boundary_conditions(all_data,dt):
    ...
    if 'T_top' in all_data.columns and 'T_bottom' in all_data.columns:
        ...
        lec_bc = lec_bc.replace('[valclt_haut]', format_value(all_data['T_top'][0]))
        lec_bc = lec_bc.replace('[valclt_bas]', format_value(all_data['T_bottom'][0]))
    elif 'TempMolo' in all_data.columns and 'Temp4' in all_data.columns:
    ...
    lec_bc = lec_bc.replace('[valcl_haut]', format_value(all_data['top'][0]))
    lec_bc = lec_bc.replace('[valcl_bas]', format_value(all_data['bot'][0]))
...
def boundary_conditions_perm(all_data):
    ...
    # Save temperature boundary conditions
    if 'T_top' in all_data.columns and 'T_bottom' in all_data.columns:
        lec_bc = lec_bc.replace('[valclt_haut]', '%70f' % all_data['T_top'][0])
        lec_bc = lec_bc.replace('[valclt_bas]', '%70f' % all_data['T_bottom'][0])
    elif 'TempMolo' in all_data.columns and 'Temp4' in all_data.columns:
        lec_bc = lec_bc.replace('[valclt_haut]', '%70f' % all_data['TempMolo'][0])
        lec_bc = lec_bc.replace('[valclt_bas]', '%70f' % all_data['Temp4'][0])
    else:
        print("Error: No temperature boundary conditions found.")
        return
    ...
    iclchgt = 0
    lec_bc = lec_bc.replace('[iclchgt]', '%1i' % iclchgt)
    lec_bc = lec_bc.replace('[valcl_haut]', '%70f' % all_data['top'][0])
    lec_bc = lec_bc.replace('[valcl_bas]', '%70f' % all_data['bot'][0])
    # Écrire les modifications dans le fichier
    f_param_lec_new.write(lec_bc)
