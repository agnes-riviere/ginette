...
c		open(unit=231,file='E_tempT_Riv.dat')
		...
		open(unit=225,file='E_t_boundaryPP.txt',iostat=ios3)
		open(unit=226,file='FODTSginette.txt',iostat=ios4)
		open(unit=227,file='E_heads.txt',iostat=ioslope)
		...
		open(unit=300,file='E_pool_cood.dat',iostat=ipool)
		open(unit=300400,file='E_riffle_cood.dat',iostat=iriff)
		endif
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
	...
	if(abs(vzp(i)-vzm(i)).gt.abs(vzm(i)/1D9)) then
	print*,'pb de vitesse z sup',vzp(i),vzp(i)
	print*,'verifiez votre etat initial'
	stop
	endif
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
	...
	if(abs(vzp(i)-vzm(i)).gt.abs(vzm(i)/1D9)) then
	print*,'pb de vitesse z inf',vzp(i)-vzm(i)
	print*, vzm(i)/1D8
	print*,'verifiez votre etat initial'
	stop
	endif
